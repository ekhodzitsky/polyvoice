...
fn configured_cap_replaces_the_default() {
    ...
    assert!(
        p.run(&[0.0_f32; 100], SampleRate::new(16000).unwrap())
            .is_ok()
    );
    assert!(
        p.run_with_timings(&[0.0_f32; 100], SampleRate::new(16000).unwrap())
            .is_ok()
    );

    let err = p
        .run(&[0.0_f32; 101], SampleRate::new(16000).unwrap())
        .unwrap_err();
    ...
    assert_eq!(
        err.to_string(),
        "audio too long: 101 samples exceeds max 100"
    );
}
...
#[test]
fn builder_accepts_a_cap_above_one_hour() {
    let p = Pipeline::builder()
        .profile(Profile::Custom)
        .max_audio_samples(MAX_AUDIO_SAMPLES * 2)
        .with_segmenter(Box::new(MockSegmenter {
            segments: Vec::new(),
        }))
        ...
        .build()
        .unwrap();
    assert_eq!(p.config().max_audio_samples, MAX_AUDIO_SAMPLES * 2);
}

#[test]
fn zero_audio_cap_accepts_only_empty_input() {
    let mut p = pipeline_with_segments(Vec::new());
    p.config.max_audio_samples = 0;
    let sr = SampleRate::new(16000).unwrap();
    assert!(p.run(&[], sr).is_ok());
    assert!(matches!(
        p.run_with_timings(&[0.0], sr),
        Err(PipelineError::AudioTooLong {
            actual_samples: 1,
            max_samples: 0
        })
    ));
