...
//!
//! Run with:
//!   cargo test --release --test hybrid_window_sweep \
...
fn hybrid_window_sweep_on_aorju() {
    let stem = "aorju";
    ...
    let pool_size = std::thread::available_parallelism()
        .map(|n| n.get())
        .unwrap_or(4);
    println!("\nWindow/hop sweep on aorju:");
    println!(
        "{:>8} {:>8} {:>10} {:>10} {:>8} {:>8} {:>10}",
        "win_s", "hop_s", "chunks", "speakers", "DER%", "miss%", "conf%"
    );

    for (win_sec, hop_sec) in [(2, 1), (2, 2), (3, 1), (3, 2), (4, 1), (4, 2), (5, 2)] {
        let win_samples = win_sec * 16000;
        let hop_samples = hop_sec * 16000;
