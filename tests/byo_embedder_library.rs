...
fn offline_custom_embedder_finds_multiple_speakers() {
    ...
    let extractor = AxisEmbedder::new(32);
...
fn streaming_custom_embedder_emits_turns_after_flush() {
    use polyvoice::streaming::StreamingParams;
    ...
    config.window.hop_secs = 0.25;
    let vad_config = VadConfig::default();
    let vad = EnergyVad::new(-40.0, sr, vad_config.frame_size);
    ...
    let mut stream =
        StreamingPipeline::with_params(vad, AxisEmbedder::new(32), config, vad_config, params)
            .expect("stream init");
    ...
    assert!(
        !stream.turns().is_empty() || stream.num_speakers() >= 1,
        "expected turns or speakers after flush"
    );
}
