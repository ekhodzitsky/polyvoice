...
mod e2e {
    use polyvoice::{
        DiarizationConfig, FbankOnnxExtractor, Pipeline, VadConfig, silero_vad::SileroVad,
    ...
    fn test_wespeaker_embedding_shape() {
        ...
        let extractor = FbankOnnxExtractor::new(&model_path, 256, 2).expect("load WeSpeaker");
    ...
    fn test_full_pipeline_onnx() {
        ...
        let extractor = FbankOnnxExtractor::new(&model_path, 256, 4).expect("load WeSpeaker");
        let mut vad = SileroVad::new(&vad_path, 512).expect("load Silero VAD");
        ...
        eprintln!(
            "Pipeline result: {} speakers, {} turns",
            result.num_speakers,
            result.turns.len()
        );
