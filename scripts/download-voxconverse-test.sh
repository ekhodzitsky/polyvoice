...
if [ "$RTTM_COUNT" -gt 50 ]; then
    ...
    curl -sL --connect-timeout 30 --max-time 300 --retry 10 --retry-delay 10 --continue-at - "$ARCHIVE_URL" -o "$TMP_TAR"
...
if [ "$WAV_COUNT" -gt 50 ]; then
    ...
    if [ -f "$ZIP_FILE" ]; then
        ...
        curl -L --connect-timeout 30 --max-time 600 --progress-bar --retry 10 --retry-delay 10 --continue-at - -o "$ZIP_FILE" "$AUDIO_URL"
